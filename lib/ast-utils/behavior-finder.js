  ...
  var visitors = {
    ...
    _initBehavior: function(node, getName) {
      ...
      if (node.expression.right.type == 'ArrayExpression') {
        var array = node.expression.right;
        var behaviors = [];
        for (var i=0; i< array.elements.length; i++) {
          if (array.elements[i].type === 'MemberExpression')
            behaviors.push(astValue.expressionToValue(array.elements[i]));
        }
        if (behaviors.length > 0)
          currentBehavior.behaviors = behaviors;
      }
    },
